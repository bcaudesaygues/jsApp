$(document).ready(function() {
	...
	*******************************/
	...
	var bindFormToObj = function(formNode) {
		var formObj = {};
	...
	****************/
	var FlowController = {
		list: function() {
			var flows = Model.prototype.findAll(Flow)
			Console.log(flows);
            flowListView.render(flows);
		},
		show: function(id) {
		...
		save: function(flowForm, callback) {
			...
			if (flowForm.id) {
				...
				_.each(flowForm, function(value, property) {
					if (flow["set_" + property] && !_.isNaN(parseInt(value))) {
	...
	****************/
	...
	var flowDetailView = {
		...
		save: function(event) {
			...
			var flowForm = bindFormToObj($("#app #flow-detail form"));
			FlowController.save(flowForm, flowDetailView.close);
		},
		bind: function() {
